...
+ (void)ensureAudioSessionWithPlayback:(BOOL)playback {
  RTCAudioSession* session = [RTCAudioSession sharedInstance];
  RTCAudioSessionConfiguration* config = [RTCAudioSessionConfiguration webRTCConfiguration];
  
  if (playback) {
    // WebRTC requires PlayAndRecord category to receive audio properly, even for RecvOnly
    // However, we don't create local audio tracks, so microphone won't be accessed
    // This allows receiving audio without actually using the microphone hardware
    if (session.category != AVAudioSessionCategoryPlayAndRecord &&
      ...
      config.categoryOptions =
          ...
          AVAudioSessionCategoryOptionAllowBluetoothA2DP |
          AVAudioSessionCategoryOptionAllowAirPlay;
      // Note: We don't add DefaultToSpeaker option to avoid forcing speaker mode
      
      [session lockForConfiguration];
      NSError* error = nil;
      bool success = [session setCategory:config.category withOptions:config.categoryOptions error:&error];
      if (!success)
        NSLog(@"ensureAudioSessionWithPlayback[true]: setCategory failed due to: %@", error);
      
      // Use WebRTC's default mode (usually VideoChat or VoiceChat)
      // This is important for WebRTC audio processing
      success = [session setMode:config.mode error:&error];
      if (!success)
        NSLog(@"ensureAudioSessionWithPlayback[true]: setMode failed due to: %@", error);
      
      // Activate the session - this is crucial for WebRTC to receive audio
      success = [session setActive:YES error:&error];
      if (!success)
        NSLog(@"ensureAudioSessionWithPlayback[true]: setActive failed due to: %@", error);
      else
        NSLog(@"ensureAudioSessionWithPlayback: Audio session activated for receiving audio (no microphone access)");
      
      [session unlockForConfiguration];
    } else {
      // Category is already PlayAndRecord or MultiRoute, just ensure it's active
      [session lockForConfiguration];
      NSError* error = nil;
      if (![session isActive]) {
        bool success = [session setActive:YES error:&error];
        if (!success)
          NSLog(@"ensureAudioSessionWithPlayback: setActive failed due to: %@", error);
        else
          NSLog(@"ensureAudioSessionWithPlayback: Audio session activated");
      }
      [session unlockForConfiguration];
    }
  }
