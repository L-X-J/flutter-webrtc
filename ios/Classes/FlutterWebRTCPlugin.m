...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"initialize" isEqualToString:call.method]) {
    ...
    // Get peerConnection reference before moving to background queue
    RTCPeerConnection* peerConnection = self.peerConnections[peerConnectionId];
    
    if (peerConnection) {
      // Run close and cleanup operations on background queue to avoid blocking UI
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        // Close peer connection (this may be a blocking operation)
        ...
        // Return to main queue for dictionary operations and audio session management
        dispatch_async(dispatch_get_main_queue(), ^{
          // Remove from dictionary on main thread
          [self.peerConnections removeObjectForKey:peerConnectionId];
          
          // Audio session management should be on main thread
          [self deactiveRtcAudioSession];
      ...
      // If peerConnection doesn't exist, just return immediately
      [self deactiveRtcAudioSession];
      result(nil);
    }
    ...
    BOOL hasAudio = NO;
    BOOL isRecvOnlyAudio = NO;
    
    // Check if this is a RecvOnly audio transceiver without a track
    if (trackId == nil && mediaType != nil) {
      RTCRtpMediaType rtpMediaType = [self stringToRtpMediaType:mediaType];
      if (rtpMediaType == RTCRtpMediaTypeAudio && transceiverInit != nil) {
        NSString* direction = transceiverInit[@"direction"];
        if (direction != nil && [direction isEqualToString:@"recvonly"]) {
          isRecvOnlyAudio = YES;
        }
      }
    }
    ...
    if (trackId != nil) {
      ...
      // We create transceiver with SendRecv but without a track, so no actual sending occurs
      if (isRecvOnlyAudio) {
        // Create transceiver with SendRecv direction but no track
        // This makes SDP show sendrecv, but since there's no track, no actual sending happens
        ...
        NSLog(@"addTransceiver: Changed RecvOnly audio to SendRecv (no track) to ensure server sends audio");
        hasAudio = YES;
      } else {
        // Normal case: add transceiver without track
        if (transceiverInit != nil) {
          RTCRtpTransceiverInit* init = [self mapToTransceiverInit:transceiverInit];
          transceiver = [peerConnection addTransceiverOfType:(rtpMediaType) init:init];
        } else {
...
- (BOOL)hasRemoteAudioTrack {
  for (RTCPeerConnection* peerConnection in _peerConnections.allValues) {
    for (RTCRtpTransceiver* transceiver in peerConnection.transceivers) {
      if (transceiver.receiver != nil && 
          transceiver.receiver.track != nil &&
          [transceiver.receiver.track.kind isEqualToString:@"audio"]) {
        return YES;
      }
    }
    // Also check remoteTracks dictionary for compatibility
...
- (void)ensureAudioSession {
  ...
  BOOL hasRemoteAudio = [self hasRemoteAudioTrack];
  
  if (hasLocalAudio) {
    // Has local audio track, need recording capability (PlayAndRecord)
    [AudioUtils ensureAudioSessionWithRecording:YES];
  } else if (hasRemoteAudio) {
    // Only has remote audio track, only need playback (Playback)
    // This allows receiving audio without accessing microphone
    [AudioUtils ensureAudioSessionWithPlayback:YES];
  } else {
    // No audio tracks, use default behavior
...
#if TARGET_OS_IPHONE
  // Only deactivate if there are no local audio tracks, no remote audio tracks, and no peer connections
  if (![self hasLocalAudioTrack] && ![self hasRemoteAudioTrack] && self.peerConnections.count == 0) {
